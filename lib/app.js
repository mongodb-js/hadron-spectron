...
class App {
  ...
  constructor(root, appRoot) {
    ...
    this.app = new Application({
      ...
      chromeDriverLogPath: path.join(
        this.root,
        `hadron-spectron_${logId}_chromedriver.log`
      ),
      webdriverLogPath: path.join(
        this.root,
        `hadron-spectron_${logId}_webdriver`
      )
    });
    debug('Created spectron.Application', this.app.getSettings());
  }
  ...
  launch(addCustomCommands) {
    debug('launching! waiting for app.start...');
    return this.app
      .start()
      .then(() => {
        debug('app.start() promise resolved');
      ...
      .catch(error => {
        debug('hadron-spectron: App failed to launch due to error:', error);
        /* eslint no-console:0 */
        console.error(
